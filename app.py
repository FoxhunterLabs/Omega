"""
Omega – Unified Autonomous Oversight Kernel (single-file demo)

Run with:
streamlit run app.py

This is a monolithic, interview-grade demo that fuses:
...
- OmegaEngine: orchestrator tying everything into one oversight console
...
# ============================================================================
# Low-level primitives: hashing, time, utilities
# ============================================================================
...
"""
rng = random.Random(seed ^ world.tick)
# Slight random walk on drift and stability
...
stability_norm = 1.0 - min(1.0, max(0.0, (self.cfg.stability_stop_min - stability) / 60.0))
...
- maps risk band → proposed_action
- enforces mode semantics
- injects human-gating near boundaries
"""

def __init__(self, invariants: Invariants, mode: str) -> None:
...
# 1) Map risk band to proposed action
band = risk.band
...
if abs(v.drift_deg) > self.invariants.drift_max_live:
invariants_violated.append("drift_exceeds_live_max")
if v.stability < self.invariants.stability_min_live:
invariants_violated.append("stability_below_live_min")
if tick_ms > self.invariants.max_tick_ms:
...
# 3) Human gating near live boundary
if self.mode == GovernorMode.LIVE and band in ("HOLD", "STOP"):
requires_gate = True
human_gate_id = self._pending_gate_id or self._new_gate_id()
reasons.append(
{
...
# 4) Effective action considering mode
...
def apply_gate(self, gate_id: str, approved: bool) -> Dict[str, Any]:
if self._pending_gate_id != gate_id:
return {
...
# For the demo we just clear the pending gate; actual action wiring is logged by
OmegaEngine.
self._pending_gate_id = None
return result
...
class AvalonEngine:
def __init__(self, audit: AuditSpine) -> None:
self.audit = audit
...
def run(self, scenario: str, world: WorldState, risk: RiskPacket, decision: Decision) -> Dict[str,
Any]:
self.run_id += 1
run_id = self.run_id
...
judge_scores = [j.score(scenario, text, {"disagreement": disagreement}) for j in
self.judges]
merged = {
...
if scores:
winner_name = max(scores.keys(), key=lambda n: (scores[n]["overall"], n))
...
proposal = self._build_proposal(run_id, winner_name, winning_response, risk, decision)
...
max_tick_ns=50_000_000, # 50 ms
)
self.kernel = SafetyKernel(self.risk_cfg)
self.governor = Governor(invariants, mode=mode)
self.memory = MemoryEngine()
self.avalon = AvalonEngine(self.audit)
...
# 3) governor
# we only know tick duration after we finish, but we pass a provisional value now; we
correct logic below
provisional_ms = 0.0
...
# Re-evaluate invariants with actual tick duration for logging & memory (decision already
logged once).
...
"human_gate" if decision_for_log.requires_human_gate else "auto_envelope",
]
summary = (
f"Tick {tick_index} in {self.world.mode} mode; risk band {risk.band}; "
f"action={decision_for_log.action}; gate={decision_for_log.requires_human_gate}"
)
frame = self.memory.add_frame(
tick=tick_index,
mode=self.world.mode,
risk_band=risk.band,
human_gate=decision_for_log.requires_human_gate,
summary=summary,
key_topics=key_topics,
)
...
# ============================================================================
# Streamlit UI – Omega oversight console
# ============================================================================

st.set_page_config(
page_title="Omega – Unified Autonomous Oversight Kernel",
layout="wide",
)
...
st.title("Omega – Unified Autonomous Oversight Kernel")
st.caption(
...
height=140,
placeholder="Example: Supervise an autonomous mining haul truck fleet under human-gated
control...",
)
...
st.metric("Risk band", band_color, help=str(risk.features))
...
st.info("Envelope is already in HOLD/STOP band but no live human gate required in this
mode.")
else:
...
result = omega.governor.apply_gate(decision.human_gate_id or "",
approved=approve)
omega.audit.log(
"human_gate_decision",
{
...
st.success("Decision recorded in audit log. Omega does not actuate; this is log-only.")
else:
...
st.markdown("---")
st.caption(
"Omega demo – single-file, deterministic oversight kernel. "
